...
###############################################################
# use case: TSMC 2022 FIRM VALUE NT$185.71 billion; 2022 Interest rates in Taiwan: 1.5; sigma(annual volatility) : 0.33


#################################################

### Cholskey Function (https://en.wikipedia.org/wiki/Cholesky_decomposition)
...
#   This method allows us to build the path in 1 unit time increments
...
Merton = function(n, mu, sigma, lambda, a, b, startprice, seed = 1) {
  ...
  # sigma - covariance matrix of the stocks; volatility 
...
#---------------------------------------------------------------------------
## Merton with Cauchy Jumps (https://en.wikipedia.org/wiki/Cauchy_process)
...
# GARCH(1,1) # generalized auto-regressive conditional heteroskedasticity (https://nccur.lib.nccu.edu.tw/bitstream/140.119/35777/6/803506.pdf) for moving average method for the volatility calculation
garch = function(n, theta, lambda, w, correlation, starting, seed){
  ...
  for (i in 2:n){
    Sigma2[1, i] = Sigma2[1, i-1] + theta1 * (w1 - Sigma2[1, i-1])/18571 + sqrt(2*lam1*theta1*Sigma2[1, i-1]/18571)*Z[3, i-1]
    Sigma2[2, i] = Sigma2[2, i-1] + theta2 * (w2 - Sigma2[2, i-1])/18571 + sqrt(2*lam2*theta2*Sigma2[2, i-1]/18571)*Z[4, i-1]    
  }
  ...
  for (i in 2:n){
    ...
    P[i,] = chol(Sigma) %*% Z[1:2, i-1] * sqrt(1/18571) + P[i-1,]
  }
  return(exp(P))
}
...
### demo
# TSMC
garch_TSMC = function(n, theta, lambda, w, correlation, starting, seed){
  lam1 = lambda[1]
  lam2 = lambda[2]
  w1 = w[1]
  w2 = w[2]
  ...
  for (i in 2:n){
    Sigma2[1, i] = Sigma2[1, i-1] + lam1 * (w1 - Sigma2[1, i-1])/18571 + sqrt(2*lam1*theta1*Sigma2[1, i-1]/18571)*Z[3, i-1]
    Sigma2[2, i] = Sigma2[2, i-1] + lam2 * (w2 - Sigma2[2, i-1])/18571 + sqrt(2*lam2*theta2*Sigma2[2, i-1]/18571)*Z[4, i-1]    
  }
...
#---------------------------------------------------------------------------
## Variance Gamma : interest rate used to discount the safety level Ht (it is a constant value).
